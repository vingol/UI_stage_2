...
class PlotCanvas(FigureCanvas):
    ...
    def show_plot(self, df, df2, cap):
        ...
        self.axes.set_title('待判断电站功率与晴空模型对比')
    ...
    def show_dist(self, df):
        ...
        self.axes.tick_params(top=False,right=False)
        # self.axes.spines['right'].set_visible(False)
        self.axes.legend(['波动置信度'])
        self.axes.set_title('待判断区域内的光伏功率波动置信度')
        self.draw()
...
class Ui_MainWindow_solar_confidence(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label.setText(_translate("MainWindow", "光伏波动预警置信度评估"))
    ...
    def show_solar_confidence(self):
        ...
        print('正在进行光伏波动预警置信度评估，请稍后...')

        # load_data
        try:
            sample_time = pd.to_datetime(start_time)  # 目标时刻点（认为是最近历史时刻）
        except NameError:
            sample_time = pd.to_datetime(datetime.datetime(2018, 6, 19, 12, 15))
        ...
        P_fit = pd.read_csv('Time_series_data/JILIN/Solar/P_fit/' + str(ID_sta) + '.csv', index_col=0, parse_dates=True)
        ...
        power_to_show = power.loc[sample_time + datetime.timedelta(minutes=15 * 1):sample_time + datetime.timedelta(
            minutes=15 * 16)]
        P_fit_to_show = P_fit.loc[sample_time + datetime.timedelta(minutes=15 * 1):sample_time + datetime.timedelta(
            minutes=15 * 16)]
        ...
        if points:
            ...
            for i in range(len(power_to_show)):
                ...
                confidences.append(1-len(df_chosen[df_chosen['rmse'] < 1]) / len(df_chosen))
            ...
            with open('data_to_draw.pkl', 'wb') as f:
                pickle.dump([power_to_show, P_fit_to_show, df_confidence], f)

            self.m.show_plot(power_to_show, P_fit_to_show, power.max()[0])
